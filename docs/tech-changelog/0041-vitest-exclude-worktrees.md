...
`vite.config.ts`의 `exclude` 두 곳에서 `WORKTREE_GLOB` 참조와 상수 선언을 제거하고, import를 `vite`의 `defineConfig` + `/// <reference types="vitest/config" />` 조합으로 되돌린다.
