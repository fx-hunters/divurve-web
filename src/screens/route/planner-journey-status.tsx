...
export function PlannerJourneyStatus({ goal, onContinue, onBack, onHistory }: PlannerJourneyStatusProps) {
  return (
    ...
    <ProgressBar ratio={goal.progressPercent} label={goal.progressLabel} />
      <div className="planner-api-journey__buttons"><button type="button" className="planner-api-journey__secondary" onClick={onBack}>목표 다시 고르기</button><button type="button" className="planner-api-journey__secondary" onClick={onHistory}>계획 이력 보기</button><button type="button" className="planner-api-journey__primary" onClick={onContinue}>계획 Curve 보기</button></div>
    </section>
  );
}
