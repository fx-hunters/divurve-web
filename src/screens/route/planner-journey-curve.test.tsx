...
describe("PlannerJourneyCurve", () => {
  ...
  it("renders empty curve without a native SVG title tooltip", () => { renderCurve(null, [], { path: "", nodes: [], destination: null }); expect(screen.queryByRole("status")).not.toBeInTheDocument(); expect(screen.getByRole("img", { name: /계획 경로/ })).toHaveAccessibleDescription(/환율 차트가 아닌 계획 회차/); expect(document.querySelector("title")).toBeNull(); });
